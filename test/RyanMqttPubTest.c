...
static void RyanMqttPublishEventHandle(void *pclient, RyanMqttEventId_e event, const void *eventData)
{
    ...
    case RyanMqttEventData:
    {
        ...
        {
            rlog_e("pub测试收到qos等级不一致 exportQos%d", exportQos, msgData->qos);
...
static RyanMqttError_e RyanMqttPublishHybridTest(int32_t count, uint32_t delayms)
{
    ...
    for (int32_t i = 0; i < count; i++)
    {
        RyanMqttError_e pubResult = RyanMqttPublish(client, "testlinux/pub", pubStr, pubStrLen, i % 3, RyanMqttFalse);
        if (RyanMqttSuccessError != pubResult)
        {
    ...
    for (int32_t i = 0;; i++)
    {
        ...
        if (pubTestPublishedEventCount == sendNeedAckCount && pubTestPublishedEventCount == pubTestDataEventCountNotQos0)
            break;

        if (i > 300)
        {
            rlog_e("QOS测试失败, PublishedEventCount: %d, pubTestDataEventCountNotQos0: %d", pubTestPublishedEventCount, pubTestDataEventCountNotQos0);
            result = RyanMqttFailedError;
            goto __exit;
        }
