...
static void *concurrentPublishThread(void *arg)
{
	...
	for (int i = 0; i < MESSAGES_PER_THREAD; i++)
	{
		...
		result = RyanMqttPublish(testData->client, topic, payload, strlen(payload),
...
static RyanMqttError_e multiClientConcurrentTest(void)
{
	...
	// 初始化测试控制结构
	memset(&g_testControl, 0, sizeof(g_testControl));
	...
	{
		memset(&testThreads[i], 0, sizeof(ThreadTestData_t));
