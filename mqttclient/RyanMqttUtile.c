 ...
 */
RyanMqttBool_e RyanMqttMatchTopic(const char *topic, const uint16_t topicLength, const char *topicFilter,
 ...
 * @return RyanMqttError_e
 */
RyanMqttError_e RyanMqttMsgHandlerFind(RyanMqttClient_t *client, const char *topic, uint16_t topicLen,
				       RyanMqttBool_e topicMatchedFlag, RyanMqttMsgHandler_t **pMsgHandler)
{
	...
	RyanListForEachSafe(curr, next, &client->msgHandlerList)
	{
		...
		if (RyanMqttFalse == RyanMqttMsgIsMatch(msgHandler, topic, topicLen, topicMatchedFlag))
		{
			continue;
		}
...
void RyanMqttMsgHandlerFindByPackId(RyanMqttClient_t *client, const char *topic, uint16_t topicLen, uint16_t packetId,
				    RyanMqttBool_e isPacketIdEqual)
{
	RyanList_t *curr, *next;
	RyanMqttMsgHandler_t *msgHandler = NULL;
	...
	RyanListForEachSafe(curr, next, &client->msgHandlerList)
	{
		...
		if (RyanMqttFalse == isPacketIdEqual)
		{
			if (msgHandler->packetId != packetId)
			{
				continue;
			}
		}
		...
		if (RyanMqttFalse == RyanMqttMsgIsMatch(msgHandler, topic, topicLen, RyanMqttFalse))
		{
			continue;
		}
					 ...
					 RyanMqttAckHandler_t **pAckHandler, RyanMqttBool_e isPreallocatedPacket)
{
	...
	RyanMqttTimerCutdown(&ackHandler->timer,
			     client->config.ackTimeout); // 超时内没有响应将被销毁或重新发送
...
uint32_t RyanMqttTimerRemain(RyanMqttTimer_t *platformTimer)
{
	...
	// Calculate elapsed time handling wraparound
	...
	{
		// Handle wraparound: time wrapped from UINT32_MAX to 0
