...
static int MqttState(int argc, char *argv[])
{
	...
	default:
		RyanMqttLog_e("Invalid client state: %d", clientState);
		str = "未知状态";
		break;
	}
...
static int MqttConnect(int argc, char *argv[])
{
	...
	// 设置遗嘱消息
	result = RyanMqttSetLwt(client, "pub/test", "this is will", RyanMqttStrlen("this is will"), RyanMqttQos0, 0);
	RyanMqttCheck(RyanMqttSuccessError == result, result, RyanMqttLog_d);
...
static int Mqttpublish(int argc, char *argv[])
{
	...
	{
		if (RyanMqttSuccessError == RyanMqttPublish(client, topic, payload, RyanMqttStrlen(payload), qos, 0))
		{
			pubCount++;
		}
		delay(delayTime);
...
static const struct RyanMqttCmdDes cmdTab[] = {
	...
	// {"destroy",     "mqtt销毁客户端             params: null", MqttDestroy},
...
MSH_CMD_EXPORT_ALIAS(RyanMqttMsh, mqtt, RyanMqtt示例程序);
#endif
